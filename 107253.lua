...
local S = { Insta=false, ESP=false, Hatch=false, EquipBest=false, Claim=false, Fly=false }
local alive = true
local flySpeed = 50
local flySpeedStep = 5
...
local m = Instance.new("Frame"); m.Size = UDim2.fromOffset(292,460); m.Position = UDim2.fromOffset(40,80)
...
tt.TextXAlignment = Enum.TextXAlignment.Left; tt.Text = "POTATO - STEAL AN EGG v5 + Fly"; tt.Parent = bar
...
flyFrame.Size = UDim2.new(1,0,0,36)
flyFrame.BackgroundColor3 = Color3.fromRGB(26,26,34)
flyFrame.BorderSizePixel = 0
flyFrame.LayoutOrder = od
flyFrame.Parent = lf
Instance.new("UICorner", flyFrame).CornerRadius = UDim.new(0,6)

local flyLayout = Instance.new("UIGridLayout")
flyLayout.Parent = flyFrame
...
flyLayout.CellPadding = UDim.new(0,5)
flyLayout.CellSize = UDim2.new(0,70,0,26)

local flyToggle = Instance.new("TextButton")
flyToggle.Size = UDim2.new(0,70,0,26)
flyToggle.BackgroundColor3 = Color3.fromRGB(40,40,55)
flyToggle.BorderSizePixel = 0
flyToggle.Font = Enum.Font.GothamBold
flyToggle.TextSize = 12
flyToggle.TextColor3 = W
flyToggle.Text = "Fly OFF"
flyToggle.Parent = flyFrame
Instance.new("UICorner", flyToggle).CornerRadius = UDim.new(0,4)

local function updateFlyToggle()
    flyToggle.Text = S.Fly and "Fly ON" or "Fly OFF"
    flyToggle.BackgroundColor3 = S.Fly and Color3.fromRGB(30,100,30) or Color3.fromRGB(40,40,55)
end
updateFlyToggle()
...
flyToggle.MouseButton1Click:Connect(function()
    ...
    if not S.Fly then
        ...
        if hu then
            ...
            pcall(function() hu.WalkSpeed = 16 end)
        end
    end
end)
...
RunS.Heartbeat:Connect(function()
    ...
    -- FLY
    if S.Fly and hu and hp then
        pcall(function()
            hu.PlatformStand = true
            hu.WalkSpeed = flySpeed
            if hp.Velocity.Y < -0.1 then
                hp.Velocity = Vector3.new(hp.Velocity.X, 0, hp.Velocity.Z)
            end
        end)
    elseif hu then
        pcall(function()
...
getgenv().__POTATO_EGG = { S = S, u = u, AREA = AREA, flySpeed = flySpeed }
warn("[POTATO STEAL AN EGG v5] loaded - toggle Insta Steal, then press E. Unload: End. Fly: toggle button + arrow keys.")